...
base_folder="/home/vincenzo/romeo/data/output/synthetic"
...
PIDS=$(pgrep -f "com.vincenzogulisano.usecases.synthetic.QuerySynthetic")
...
args="-s ${exp_folder} -i ${input_file} -l ${duration} -wa ${wa} -ws ${ws} -t REALRATE -d ${d}"

mvn clean compile package exec:java -Dexec.mainClass="com.vincenzogulisano.usecases.synthetic.QuerySynthetic" -Dexec.args="${args}" > ${exp_folder}/spe.log 2>&1 &

sleep 5

# Use pgrep to find the PID of the Java process
JVM_PID=$(pgrep -f "com.vincenzogulisano.usecases.synthetic.QuerySynthetic")
